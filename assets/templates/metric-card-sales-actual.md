...
产品维度（4，四层分类）：
- `product_type`（**品类**，如 手动轮椅 / 电动轮椅 / 助行器，精确匹配）
...
- `remark`（**车型**，短字符串；默认精确匹配，必要时 LIKE）
  - 示例值：`SPIRIT X4` / `Y069` / `A100`
  - 用户给的是简短车型名/型号（字母数字组合）时，定位到此字段
  - **典型查询**：用户问"助行器 X4" → `product_type = '助行器' AND remark = 'SPIRIT X4'`（品类 = 助行器，车型 = SPIRIT X4）
  - 如果用户只给车型一部分（如"X4"而不是"SPIRIT X4"）：用 `remark LIKE '%X4%'` 兜底
