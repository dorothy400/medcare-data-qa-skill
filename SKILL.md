...
3. 在构建或选择查询之前，阅读 [references/query-contract.md](references/query-contract.md)。
   ...
   **🚫 严禁使用 plotly / matplotlib / seaborn 等任何 Python 绘图库生成 PNG 再上传，零例外、零兜底、零理由。** 必须使用飞书原生 `chart` 元素（基于 VChart）直接在卡片里渲染。即使下列情况也**不**允许跑回 plotly：
   - "查询太复杂，VChart spec 写不下来"——见下方"拆图策略"
   - "多客户 × 多年 × 多指标，一张图塞不下"——拆成多张 VChart 子图
   - "VChart 我不熟，plotly 更顺手"——不接受
   - "时间紧，先 PNG 应急"——同样不接受

   原因（不再赘述，违反会被识别为质量缺陷）：
   - PNG 路径破坏单卡片原则（拆出多条消息）
   - 服务端中文字体渲染常出问题
   - 用户失去点击放大 / 导出 / 交互能力
   - 偏离规范本身就让 audit / debug 时定位问题变难

   **"5/23 22:02 卡片"案例**（反面教材，禁止重复）：worker 处理"9OTTO & 9COHB 近三年同期接单分析"时，用 plotly 画 3 张 PNG 上传——这是**质量事故**，不是合理变通。下次类似复杂场景必须按下方"拆图策略"走。

   **拆图策略（复杂查询的唯一正确兜底）**：
   - 如果一张 VChart 图塞太多 series 会变乱（如 3 年 × 2 客户 × 月度 + 年度 = 太多线），**拆成多张 VChart 子图**，每张图聚焦一个粒度或一个指标维度。
   - 拆图轴：按**时间粒度**拆（一张月度趋势 + 一张年度趋势）、按**指标类型**拆（一张绝对值 + 一张环比% + 一张同比%）。
   - 不要按**分组维度**拆（不要"客户 A 一张图、客户 B 一张图"——破坏对比能力，参见 chart-conventions.md § 2）。
   - 例：用户问"9OTTO & 9COHB 近三年同期分析" → 输出 **3 张 VChart 子图，全部放在同一张卡片里**：
     1. **月度订单趋势**（折线图）：2 个客户作为 series，按 24/25/26 三年的月份展示
     2. **年度合计对比**（折线图）：2 个客户作为 series，横轴 24/25/26 三年
     3. **同比 / 环比%**（折线图）：2 个客户的 yoy% / mom% 趋势
   - **3 张图都必须用 `chart` 元素**，全部嵌在同一张 interactive card 的 `body.elements` 里。绝不允许任何一张降级成 PNG。
