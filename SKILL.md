...
| 粒度 | month | day |
| 字段丰富度 | 8 个 filter | 17 个 filter（多了产品三层分类/单价/件数/工厂/公司/区域/国家/客户/船期等） |

**实际销售表的 17 个 filter 字段速查**（详见 `assets/templates/metric-card-sales-actual.md`）：
...
| 产品 — 三层分类（3） | `product_type`（大类，=）/ `material`（材料，=）/ `material_name`（**模糊查 LIKE '%xxx%'**）|
| 订单明细 — 数值（2） | `price`（单价 USD，范围筛选）/ `quantity`（件数，范围筛选 + 可 sum）|
| 地理（2） | `area`（区域）/ `country`（国家）|

**关键约束**：
- 用户问"某个具体产品"且带规格 / 型号 / 颜色 / 尺寸时，**必须用 `material_name LIKE '%xxx%'`** 做模糊查询，不能用 `=`。例：`轮椅 Y069型` → `WHERE material_name LIKE '%Y069%'`。
- 只有当用户明确说"产品大类"或给的是粗略品类名（如"手动轮椅" vs "电动轮椅"）时，才用 `product_type = '...'` 精确匹配。
