...
| 产品 — 四层分类（4） | `product_type`（大类，=）/ `material`（材料，=）/ `material_name`（**模糊 LIKE**）/ `remark`（型号代码，默认=）|
| 订单明细 — 数值（2） | `price`（单价 USD，范围筛选）/ `quantity`（件数，范围筛选 + 可 sum）|
| 地理（2） | `area`（区域）/ `country`（国家）|

**产品查询路由约束**：
- 用户给**短型号代码**（字母数字组合，如 `X4` / `Y069` / `A100` / `SPIRIT X4`）→ **`remark`** 字段，默认精确匹配 `remark = '...'`；若用户只给一半（如 `X4` 而不是 `SPIRIT X4`），用 `remark LIKE '%X4%'`。
  - 典型查询：用户问"助行器 X4" → `WHERE product_type = '助行器' AND remark = 'SPIRIT X4'`
- 用户给**长描述**（带颜色 / 尺寸 / 材质等修饰词，如"智能可调节轮椅" / "24 寸轮椅"）→ **`material_name`** 字段，**必须**用 `LIKE '%xxx%'`，不能用 `=`。
- 用户给**粗略品类名**（如"轮椅" / "助行器"）→ **`product_type`** 字段，精确匹配。
